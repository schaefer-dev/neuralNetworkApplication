...
\begin{enumerate}[a)]
    ...
    \item
        % TODO
         \begin{figure}[H]
            \centering
            \includegraphics[width=300pt]{pictures/pca_a.png}
            \caption{gradient Descent with $\epsilon = 0.1$}
            \label{fig:pcaa}
        \end{figure}
         \begin{figure}[H]
            \centering
            \includegraphics[width=300pt]{pictures/pca_b.png}
            \caption{gradient Descent with $\epsilon = 0.1$}
            \label{fig:pcab}
        \end{figure}
        The two plots are given in \autoref{fig:pcaa} and \autoref{fig:pcab}. As you can see in this picture the reconstructed points are scaled up a lot, i.e. we perhaps have to normalize the resulting eigenvectors. Furthermore, this is example shows it is not possible to reconstruct the points if we have only 4 training instances because we need much more training examples to find an accurate encoding and decoding. And we lose information by reducing the dimension.
\end{enumerate}
...
\begin{enumerate}[a)]
    ...
    \item
        ...
        \begin{figure}[H]
            ...
            \label{fig:fig7}
        \end{figure}
        you can see the steps of Newtons Method displayed in Figure \autoref{fig:fig7}.\\
        We coverge to the desired value step by step as it is usual by using the newton method. Furthermore, it converges faster than the other method.
